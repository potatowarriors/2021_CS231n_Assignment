...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    H = h0.shape[1]
    h_set = {} #h 꺼내쓰기 편하게 빈 딕트타입 배열 하나 생성
    h_set['h0'] = h0 #함수의 매개변수 최초의 h0을 별도로 하나 지정
    step_cache_save = []
    #print(x)
    #print(x[:,1,:])
    h = np.zeros((N,T,H))
    for t in range(T):
      x_temp = x[:,t,:].reshape(N,D) # temporal한 녀석을 반복문으로 하나씩 꺼내와 matrix형태로 재가공해준다
      h_set['h' + str(t+1)], step_cache = rnn_step_forward(x_temp, h_set['h' + str(t)], Wx, Wh, b) # 매트릭스 가공된 놈을 반복적으로 step forward에 넣어준다
      h[:,t,:] = h_set['h' + str(t+1)] # zero로 초기화한 h에 차곡 차곡 값을 담아준다.
      step_cache_save.append(step_cache) # backprop에 필요한 cache는 빈 리스트에 차곡차곡 쌓아준다. (stack 해서 팝하는게 나을라나?)
    
    cache = step_cache_save
...
def rnn_backward(dh, cache):
    ...
    __, __, __, Wx, __ = cache[0]
    D = Wx.shape[0] # 캐쉬에서 꺼내와서 D를 정의해준다.
    dx = np.zeros((N,T,D))
    dh0 = np.zeros((N,H))
    dWx = np.zeros((D,H))
    dWh = np.zeros((H,H))
    ...
    for t in range(T-1, -1, -1) :
      dh_temp = dh[:,t,:].reshape(N,H) # 특정 t에 대해 꺼내오고 reshape시켜준다
      dx_out, dprev_h_out, dWx_out, dWh_out, db_out = rnn_step_backward(dh_temp + dprev_h, cache[t])
      dx[:,t,:] += dx_out
      dprev_h = dprev_h_out
      dWx += dWx_out
      dWh += dWh_out
